...
class RawDataService {
  ...
  async getDashboardStatistics(): Promise<{
    ...
    try {
      ...
      const todayImports = recentImports.filter(imp => new Date(imp.importDate) >= today).length

      const processingImports = recentImports.filter(imp => imp.status === 'PROCESSING').length
...
export const validateExcelFile = (file: File, tableType: string) => rawDataService.validateExcelFile(file, tableType)

export const getRawDataStatistics = (tableName: string) => rawDataService.getImportStatistics(tableName)

export const getRecentImports = (limit?: number) => rawDataService.getRecentImports(limit)

export const getLN01History = (sourceId: string) => rawDataService.getLN01History(sourceId)

export const getGL01History = (sourceId: string) => rawDataService.getGL01History(sourceId)

export const getLN01Snapshot = (snapshotDate: string) => rawDataService.getLN01Snapshot(snapshotDate)

export const getGL01Snapshot = (snapshotDate: string) => rawDataService.getGL01Snapshot(snapshotDate)

export const cleanupOldData = (tableName: string, retainMonths?: number) =>
  rawDataService.cleanupOldData(tableName, retainMonths)

export const getDashboardStatistics = () => rawDataService.getDashboardStatistics()
