<template>
  <div class="units-view">
    ...
    <div style="margin-bottom: 18px; display: flex; align-items: center; gap: 12px;">
      ...
      <select id="viewMode" v-model="viewMode" style="min-width: 140px; padding: 6px 10px; border-radius: 4px; border: 1px solid #ced4da;">
...
const handleSubmitUnit = async () => {
  ...
  let parentIdToSubmit = currentUnit.value.parentUnitId !== null && currentUnit.value.parentUnitId !== '' ? Number(currentUnit.value.parentUnitId) : null;
  ...
  console.log("Dữ liệu sẽ được kiểm tra và gửi đi (unitDataForSubmission):", JSON.parse(JSON.stringify(unitDataForSubmission)));
  ...
  console.log('Danh sách đơn vị hiện có:', unitStore.allUnits.map(u => ({id: u.id, code: u.code, type: u.type, parentUnitId: u.parentUnitId})));
  ...
  console.log(
    "VALIDATION PASS (Client-side): Dữ liệu hợp lệ, tiến hành submit."
  );
  ...
  if (
    ...
    try {
      ...
      if (error && error.response) {
        ...
        formError.value = error.response.data?.message || error.response.data || error.message || 'Lỗi không xác định từ backend.';
      } else {
...
const startEditUnit = (unit) => {
  ...
  currentUnit.value.parentUnitId =
    unit.parentUnitId === null ? null : Number(unit.parentUnitId);
  console.log(
    "Dữ liệu được nạp vào form sửa (startEditUnit):",
    JSON.parse(JSON.stringify(currentUnit.value))
  );
};
