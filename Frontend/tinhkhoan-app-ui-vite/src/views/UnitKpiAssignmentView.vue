          ...
          <table v-if="availableKpiIndicators.length > 0" class="table-agribank">
            ...
            <tbody>
              ...
              >
                ...
                <td style="text-align: center">
                  <strong class="badge-agribank badge-primary">{{ totalMaxScore }}</strong>
                </td>
                <td>
                  <strong class="text-secondary">{{ getFilledTargetsCount() }} mục tiêu</strong>
                </td>
                <td style="text-align: center">
...
async function onBranchChange() {
  ...
  try {
    ...
    const branchTables = kpiTables.value.filter(t => t && t.Category === 'CHINHANH')
...
watch(
  [selectedPeriodId, selectedBranchId],
  ([newPeriodId, newBranchId]) => {
