<template>
  <div class="payroll-report-container">
    ...
    <div class="form-group">
      ...
      <input v-model="search" placeholder="Tìm kiếm tên/mã NV..." style="padding:4px 8px;border-radius:4px;border:1px solid #ccc;min-width:180px" />
...
const filteredPayroll = computed(() => {
  ...
  if (search.value) {
    ...
    result = result.filter(row =>
      row.fullName?.toLowerCase().includes(s) || row.employeeCode?.toLowerCase().includes(s)
    );
  }
  if (unitFilter.value) {
...
const exportExcel = () => {
  ...
  window.open(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5055/api'}/Reports/employee-summary-excel?khoanPeriodId=${selectedPeriod.value}`);
};
const exportPdf = () => {
  if (!selectedPeriod.value) return;
  toast.info('Đang xuất file PDF...');
  window.open(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5055/api'}/Reports/employee-summary-pdf?khoanPeriodId=${selectedPeriod.value}`);
};
