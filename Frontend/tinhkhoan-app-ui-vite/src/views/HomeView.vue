...
const applyTextEffects = () => {
  ...
  textElement.classList.remove(
    ...
    'weather-stormy', 'weather-snowy', 'weather-clear-night'
  );
  ...
  try {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        console.log(`📱 Device: ${isUltraSmall ? 'Ultra-Small' : isMobile ? 'Mobile' : isTablet ? 'Tablet' : 'Desktop'} ${isLandscape ? '(Landscape)' : '(Portrait)'}`);
        console.log(`📏 Viewport: ${viewportWidth}x${viewportHeight}, Container: ${containerWidth}px`);
        console.log(`📐 RealText: ${realTextWidth}px → Scaled: ${scaledWidth.toFixed(1)}px (scale: ${scale.toFixed(3)})`);
        console.log(`🎯 MaxWidth: ${maxAllowedWidth.toFixed(0)}px, Usage: ${((scaledWidth / maxAllowedWidth) * 100).toFixed(1)}%`);
