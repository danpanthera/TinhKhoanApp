  ...
  actions: {
    async fetchUnits() {
      ...
      try {
        ...
        this.units = [];
        this.error =
          "Không thể tải danh sách đơn vị. Lỗi: " +
    ...
    async createUnit(unitData) {
      ...
      try {
        ...
        if (!Array.isArray(this.units)) {
          console.warn(
            "this.units không phải là mảng khi cố gắng thêm đơn vị mới. Khởi tạo lại units."
          );
          this.units = [];
        }
      ...
      try {
        ...
        if (err.response && err.response.data) {
          if (err.response.data.errors) {
            ...
            errorMessage +=
              " Lỗi: " + (err.message || "Lỗi không xác định từ server.");
          }
        } else {
          errorMessage +=
            " Lỗi: " + (err.message || "Lỗi mạng hoặc server không phản hồi.");
        }
