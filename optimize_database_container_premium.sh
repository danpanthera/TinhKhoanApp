...
# Verify database KhoanDB
echo "🗄️ [DATABASE] Kiểm tra database KhoanDB..."
DB_EXISTS=$(sqlcmd -S localhost,1433 -U sa -P 'YourStrong@Password123' -C -Q "SELECT COUNT(*) FROM sys.databases WHERE name = 'KhoanDB'" -h -1 -W 2>/dev/null || echo "0")
if [ "$DB_EXISTS" -gt 0 ]; then
  ...
  sqlcmd -S localhost,1433 -U sa -P 'YourStrong@Password123' -C -d KhoanDB -Q "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME IN ('DP01', 'DPDA', 'EI01', 'GL01', 'GL41', 'LN01', 'LN03', 'RR01') ORDER BY TABLE_NAME" -t 10 2>/dev/null || echo "   ⚠️ Không thể kiểm tra core tables"
