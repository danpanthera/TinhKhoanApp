...
# 🎯 KhoanApp Full Stack Status Report
...
echo -e "${CYAN}🎯 KhoanApp Full Stack Status Report${NC}"
echo -e "${CYAN}=======================================${NC}"
...
if curl -s http://localhost:3000 &>/dev/null; then
    ...
    if curl -s http://localhost:3000 | grep -q "KhoanApp"; then
        echo "   🎯 App: KhoanApp detected"
    else
...
# Backend processes
BACKEND_PROCESSES=$(pgrep -f "dotnet.*KhoanApp.Api" | wc -l | tr -d ' ')
if [ "$BACKEND_PROCESSES" -gt 0 ]; then
    echo -e "✅ ${GREEN}Backend Processes: ${BACKEND_PROCESSES} running${NC}"
    pgrep -f "dotnet.*KhoanApp.Api" | head -3 | while read pid; do
        echo "   🔧 PID: $pid"
    done
else
...
if [ "$ALL_SERVICES_OK" = true ]; then
    ...
    echo -e "✅ ${GREEN}KhoanApp is ready for development${NC}"
else
