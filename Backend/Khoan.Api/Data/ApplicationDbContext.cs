...
namespace Khoan.Api.Data // Sử dụng block-scoped namespace cho rõ ràng
{
    ...
    {
        private readonly TemporalMappingOptions _temporalOptions;

        public ApplicationDbContext(DbContextOptions<ApplicationDbContext> options, IOptions<TemporalMappingOptions>? temporalOptions = null) : base(options)
        {
            _temporalOptions = temporalOptions?.Value ?? new TemporalMappingOptions();
        }
    ...
    // ✅ Standardize EI01 on DataTables model
    public DbSet<DataTables.EI01> EI01 { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // REMOVED: DP01Entity mapping to avoid duplicate temporal mapping to the same DP01 table.
            // We standardize on DataTables.DP01 as the canonical entity for the DP01 table.
            // If repository-layer DP01Entity is needed later, map it to a separate view/table or mark as [NotMapped].

            // EI01 Temporal Table configuration - gated by options
            if (_temporalOptions.EI01)
            {
            ...
            // 🚀 === TABLE CONFIGURATION & INDEXES ===

            // === DP01 CONFIGURATION (gated temporal) ===
            modelBuilder.Entity<DataTables.DP01>(entity =>
            {
                if (_temporalOptions.DP01)
                {
                    entity.ToTable("DP01", tb => tb.IsTemporal(ttb =>
                    {
        ...
        {
            // Cấu hình bảng DP01 đã được thiết lập cụ thể phía trên (SysStartTime/SysEndTime)
            // Tránh cấu hình lại bằng generic method (ValidFrom/ValidTo) để không xung đột
        ...
        private void ConfigureDataTableWithTemporal<T>(ModelBuilder modelBuilder, string tableName) where T : class
        {
            modelBuilder.Entity<T>(entity =>
            {
                // Cấu hình bảng thành Temporal Table với cột chuẩn SysStartTime/SysEndTime
