...
";
                ...
                {
                    using var rdr = filesCmd.ExecuteReader();
                    while (rdr.Read())
                    {
...
";
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        var st = (stateCheck.ExecuteScalar())?.ToString();
...
";
                ...
                try
                {
                    ...
                    {
                        try { ping.ExecuteScalar(); } catch { /* ignore */ }
                    }
                ...
                if (warmupEx != null)
                {
                    ...
                    {
                        var res = stateCmd.ExecuteScalar();
...
";
        ...
        // Lightweight read-only DbContext for simple endpoints to avoid temporal mapping conflicts
        builder.Services.AddDbContext<Khoan.Api.Data.LightweightReadDbContext>(options =>
        {
            options.UseSqlServer(csbNormalized.ConnectionString, sqlOptions =>
            {
