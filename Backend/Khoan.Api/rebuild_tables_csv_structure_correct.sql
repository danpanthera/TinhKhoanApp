-- ================================================================
-- REBUILD 8 CORE DATATABLES - TÔN TRỌNG CSV GỐC
-- Thứ tự: NGAY_DL + Business Columns (CSV gốc) + System/Temporal Columns
-- ================================================================
...
-- ================================================================
-- 1. DROP TẤT CẢ TABLES (Temporal + History)
-- ================================================================
...
-- ================================================================
-- 2. DP01 - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 63 Business + System/Temporal)
-- ================================================================
PRINT 'Creating DP01 table with correct column order...';
CREATE TABLE DP01 (
    -- System column FIRST (theo yêu cầu user)
    NGAY_DL DATETIME NOT NULL,

    -- 63 Business columns từ CSV DP01 (exact order from DuLieuMau)
    MA_CN NVARCHAR(50),
    ...
    -- System/Temporal columns LAST (theo yêu cầu user)
...
PRINT '✅ DP01 created with correct CSV column order.';
GO

-- ================================================================
-- 3. LN01 - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 79 Business + System/Temporal)
-- ================================================================
...
PRINT '✅ LN01 created with correct CSV column order.';
GO

-- ================================================================
-- 4. EI01 - TEMPORAL TABLE (UNIFIED STRUCTURE: NGAY_DL + 24 Business + System/Temporal)
-- Business columns: 24 columns from CSV, all NVARCHAR(200) + DATE columns as DATETIME2
-- Import policy: Only files containing "ei01" in filename
-- Structure: NGAY_DL -> Business Columns -> System/Temporal Columns
-- ================================================================
PRINT 'Creating EI01 table with unified structure (datetime2 + nvarchar(200))...';
CREATE TABLE EI01 (
    -- System column FIRST (NGAY_DL from filename - datetime2 format dd/MM/yyyy)
    NGAY_DL DATETIME2 NOT NULL,

    -- 24 Business columns từ CSV EI01 (exact order) - ALL NVARCHAR(200) + DATE as DATETIME2
    MA_CN NVARCHAR(200),
    MA_KH NVARCHAR(200),
    TEN_KH NVARCHAR(200),
    ...
    NGAY_DK_EMB DATETIME2,       -- DATE column -> datetime2 (dd/MM/yyyy)
    SDT_OTT NVARCHAR(200),
    TRANG_THAI_OTT NVARCHAR(200),
    NGAY_DK_OTT DATETIME2,       -- DATE column -> datetime2 (dd/MM/yyyy)
    SDT_SMS NVARCHAR(200),
    TRANG_THAI_SMS NVARCHAR(200),
    NGAY_DK_SMS DATETIME2,       -- DATE column -> datetime2 (dd/MM/yyyy)
    SDT_SAV NVARCHAR(200),
    TRANG_THAI_SAV NVARCHAR(200),
    NGAY_DK_SAV DATETIME2,       -- DATE column -> datetime2 (dd/MM/yyyy)
    SDT_LN NVARCHAR(200),
    TRANG_THAI_LN NVARCHAR(200),
    NGAY_DK_LN DATETIME2,        -- DATE column -> datetime2 (dd/MM/yyyy)
...
PRINT '✅ EI01 created with correct CSV column order.';
GO

-- ================================================================
-- 5. GL01 - NON-TEMPORAL TABLE (UNIFIED STRUCTURE - HEAVY FILE OPTIMIZED)
-- Structure: NGAY_DL (từ TR_TIME) -> 27 Business Columns -> 4 System Columns
-- Partitioned Columnstore for ~200MB CSV files, MaxFileSize 2GB
-- All string columns: NVARCHAR(200), REMARK: NVARCHAR(1000)
-- DATE columns: DATETIME2, Decimal columns: DECIMAL(18,2) và DECIMAL(18,6)
-- ================================================================
PRINT 'Creating GL01 table (HEAVY FILE OPTIMIZED) with unified structure...';
CREATE TABLE GL01 (
    -- System column FIRST (NGAY_DL từ TR_TIME column - datetime2 format dd/MM/yyyy)
    NGAY_DL DATETIME2 NOT NULL,

    -- 27 Business columns từ CSV GL01 (exact order) - UNIFIED TYPES
    STS NVARCHAR(200),
    NGAY_GD DATETIME2,                    -- DATE column -> datetime2 (dd/MM/yyyy)
    NGUOI_TAO NVARCHAR(200),
    DYSEQ NVARCHAR(200),                  -- Changed from BIGINT to NVARCHAR(200)
    TR_TYPE NVARCHAR(200),
    DT_SEQ NVARCHAR(200),                 -- Changed from BIGINT to NVARCHAR(200)
    TAI_KHOAN NVARCHAR(200),
    TEN_TK NVARCHAR(200),
    SO_TIEN_GD DECIMAL(18,2),             -- SO_TIEN format -> decimal(18,2) #,###.00
    ...
    TR_EX_RT DECIMAL(18,6),               -- Exchange rate with higher precision
    REMARK NVARCHAR(1000),                -- REMARK -> nvarchar(1000) as per requirement
    BUS_CODE NVARCHAR(200),
    UNIT_BUS_CODE NVARCHAR(200),
    TR_CODE NVARCHAR(200),
    TR_NAME NVARCHAR(200),
    REFERENCE NVARCHAR(200),
    VALUE_DATE DATETIME2,                 -- DATE column -> datetime2 (dd/MM/yyyy)
    DEPT_CODE NVARCHAR(200),
    TR_TIME DATETIME2,                    -- TR_TIME -> datetime2, source cho NGAY_DL
    COMFIRM NVARCHAR(200),
    TRDT_TIME DATETIME2,                  -- DATE column -> datetime2

    -- 4 System columns LAST (NO temporal for heavy file performance)
    Id BIGINT IDENTITY(1,1) PRIMARY KEY,
    CreatedAt DATETIME2 DEFAULT GETUTCDATE(),
    UpdatedAt DATETIME2 DEFAULT GETUTCDATE(),
    FileName NVARCHAR(500),
    ImportBatchId NVARCHAR(100)           -- Heavy file tracking
);
PRINT '✅ GL01 created (HEAVY FILE OPTIMIZED) with unified structure (27+4=31 total columns).';
GO

-- ================================================================
-- 6. GL41 - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 13 Business + System/Temporal)
-- ================================================================
...
CREATE TABLE GL41 (
    ...
    -- 13 Business columns từ CSV GL41 (exact order)
    MA_CN NVARCHAR(50),
...
PRINT '✅ GL41 created with correct CSV column order.';
GO

-- ================================================================
-- 7. LN03 - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 17 Business + System/Temporal)
-- ================================================================
...
CREATE TABLE LN03 (
    ...
    -- 17 Business columns từ CSV LN03 (exact order)
...
PRINT '✅ LN03 created with correct CSV column order.';
GO

-- ================================================================
-- 8. RR01 - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 25 Business + System/Temporal)
-- ================================================================
...
CREATE TABLE RR01 (
    ...
    -- 25 Business columns từ CSV RR01 (exact order)
...
PRINT '✅ RR01 created with correct CSV column order.';
GO

-- ================================================================
-- 9. DPDA - TEMPORAL TABLE (ĐÚNG THỨ TỰ: NGAY_DL + 13 Business + System/Temporal)
-- ================================================================
...
CREATE TABLE DPDA (
    ...
    -- 13 Business columns từ CSV DPDA (exact order)
...
PRINT '✅ DPDA created with correct CSV column order.';
GO

-- ================================================================
-- 10. TẠO COLUMNSTORE INDEXES (Performance optimization)
-- ================================================================
...
(NGAY_DL, STS, TAI_KHOAN, TEN_TK, SO_TIEN_GD, LOAI_TIEN, MA_KH, TEN_KH, DR_CR, VALUE_DATE, TR_TIME)
...
PRINT '✅ All columnstore indexes created successfully.';
GO

-- ================================================================
-- 11. VERIFICATION - Check table structures
-- ================================================================
PRINT '📊 VERIFICATION: Checking all 8 tables structure...';
...
PRINT '🎯 SUCCESS: All 8 core datatables rebuilt with correct CSV column structure!';
