...
# Script so sánh chi tiết và chính xác cấu trúc EI01
# Created: July 12, 2025

echo "🔍 KIỂM TRA CHI TIẾT BẢNG EI01"
echo "=============================="
...
grep -E "^\s*\[Column\(" Models/DataTables/EI01.cs | grep -v -E "(NGAY_DL|CREATED_DATE|UPDATED_DATE|FILE_NAME)" | sed 's/.*"\(.*\)".*/\1/' | nl

echo ""
echo "🔢 So sánh số lượng:"
CSV_COUNT=$(head -1 "$CSV_FILE" | sed 's/^\xEF\xBB\xBF//' | tr ',' '\n' | sed 's/\r$//' | wc -l)
MODEL_COUNT=$(grep -E "^\s*\[Column\(" Models/DataTables/EI01.cs | grep -v -E "(NGAY_DL|CREATED_DATE|UPDATED_DATE|FILE_NAME)" | wc -l)
...
MODEL_COLS=$(grep -E "^\s*\[Column\(" Models/DataTables/EI01.cs | grep -v -E "(NGAY_DL|CREATED_DATE|UPDATED_DATE|FILE_NAME)" | sed 's/.*"\(.*\)".*/\1/')
...
if [ "$CSV_COUNT" -eq "$MODEL_COUNT" ]; then
    echo "✅ KẾT LUẬN: Bảng EI01 hoàn hảo với $MODEL_COUNT business columns khớp CSV!"
else
    echo "❌ KẾT LUẬN: Cần sửa model EI01 để khớp CSV ($CSV_COUNT cột)"
fi
