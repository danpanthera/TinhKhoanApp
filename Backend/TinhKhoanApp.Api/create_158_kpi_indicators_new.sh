...
echo "🎯 TẠO 158 CHỈ TIÊU KPI CHÍNH XÁC THEO DANH SÁCH"
echo "================================================"
...
# BẢNG 11: TqHkKtnb (ID: 43) - 0 chỉ tiêu (đợi giao sau)
echo ""
...
response=$(curl -s "$API_BASE/KpiIndicators")
total_count=$(echo "$response" | jq length 2>/dev/null || echo "0")
