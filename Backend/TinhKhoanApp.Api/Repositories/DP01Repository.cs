...
namespace TinhKhoanApp.Api.Repositories
{
    ...
    public class DP01Repository : Repository<DP01>, IDP01Repository
    {
        ...
        {
            return await _dbSet
                .Where(dp01 => dp01.NGAY_DL.HasValue && dp01.NGAY_DL.Value.Date == date.Date)
                .ToListAsync();
        }
        ...
        public async Task<decimal> GetTotalBalanceByBranchAsync(string branchCode, DateTime? date = null)
        {
            ...
            if (date.HasValue)
            {
                query = query.Where(dp01 => dp01.NGAY_DL.HasValue && dp01.NGAY_DL.Value.Date == date.Value.Date);
            }
