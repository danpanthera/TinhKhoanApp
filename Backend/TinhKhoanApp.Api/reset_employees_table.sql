-- Script xóa toàn bộ để tạo lại sạch
USE TinhKhoanDB;

-- Xóa foreign key constraints
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_EmployeeKpiAssignments_Employees')
    ALTER TABLE EmployeeKpiAssignments DROP CONSTRAINT FK_EmployeeKpiAssignments_Employees;

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_Employees_Positions_PositionId')
    ALTER TABLE Employees DROP CONSTRAINT FK_Employees_Positions_PositionId;

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_Employees_Units_UnitId')
    ALTER TABLE Employees DROP CONSTRAINT FK_Employees_Units_UnitId;

-- Xóa bảng
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'EmployeeKpiAssignments')
...
-- Tạo lại bảng Employees đơn giản cho test
