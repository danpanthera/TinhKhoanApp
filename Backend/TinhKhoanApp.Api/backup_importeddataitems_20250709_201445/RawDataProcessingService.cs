...
namespace TinhKhoanApp.Api.Services
{
    ...
    public class RawDataProcessingService : IRawDataProcessingService
    {
        ...
        private decimal? GetDecimalValue(Dictionary<string, object> data, string key)
        {
            ...
            // Try direct parsing if it's already a number
            ...
            if (value is System.Text.Json.JsonElement jsonElement)
            {
                try
                {
                    ...
                    else if (jsonElement.ValueKind == System.Text.Json.JsonValueKind.String)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(stringValue) && decimal.TryParse(stringValue, out var result))
                            return result;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning("⚠️ Lỗi khi parse JSON decimal cho key {Key}: {Error}", key, ex.Message);
                    return null;
                }
            }

            // Fallback to string parsing
            var stringFallback = value?.ToString()?.Trim();
            if (!string.IsNullOrWhiteSpace(stringFallback) && decimal.TryParse(stringFallback, out var fallbackResult))
