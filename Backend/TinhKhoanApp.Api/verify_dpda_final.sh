...
# Script kiểm tra và so sánh cấu trúc bảng DPDA với CSV file
# Created: July 12, 2025

echo "🔍 KIỂM TRA CẤU TRÚC BẢNG DPDA vs CSV FILE"
echo "=========================================="

CSV_FILE="/Users/nguyendat/Documents/DuLieuImport/DuLieuMau/7808_dpda_20250331.csv"
MODEL_FILE="Models/DataTables/DPDA.cs"
...
echo "$CSV_COLS" > /tmp/csv_cols.txt
echo "$MODEL_COLS" > /tmp/model_cols.txt

if diff /tmp/csv_cols.txt /tmp/model_cols.txt > /dev/null; then
    echo "✅ PERFECT: Tất cả cột business trong model khớp với CSV!"
else
    echo "❌ KHÁC BIỆT được phát hiện:"
    echo "   CSV có nhưng Model không có:"
    comm -23 /tmp/csv_cols.txt /tmp/model_cols.txt | sed 's/^/   - /'
    echo "   Model có nhưng CSV không có:"
    comm -13 /tmp/csv_cols.txt /tmp/model_cols.txt | sed 's/^/   - /'
fi
...
if [ "$CSV_COUNT" -eq "$MODEL_COUNT" ]; then
    echo "✅ PERFECT: Bảng DPDA có đủ $MODEL_COUNT cột business theo file CSV!"
