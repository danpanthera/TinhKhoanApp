...
CREATE TABLE [RawDataImports] (
    ...
    [ImportedBy] nvarchar(max) NOT NULL,
    ...
    [IsArchiveFile] bit NOT NULL,
    [ExtractedFilesCount] int NOT NULL,
    CONSTRAINT [PK_RawDataImports] PRIMARY KEY ([Id])
);
GO
...
CREATE TABLE [FinalPayouts] (
    ...
    [CompletionFactor] decimal(18,2) NULL,
    [Note] nvarchar(max) NULL,
...
CREATE INDEX [IX_ImportedDataItems_ImportedDataRecordId] ON [ImportedDataItems] ([ImportedDataRecordId]);
GO
...
VALUES (N'20250621132903_DashboardTablesCreated', N'8.0.5');
GO
...
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ImportedDataRecords]') AND [c].[name] = N'CompressedData');
IF @var0 IS NOT NULL EXEC(N'ALTER TABLE [ImportedDataRecords] DROP CONSTRAINT [' + @var0 + '];');
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ImportedDataRecords]') AND [c].[name] = N'CompressionRatio');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [ImportedDataRecords] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [ImportedDataRecords] DROP COLUMN [CompressionRatio];
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250623140553_CreateRawDataImportsTable', N'8.0.5');
GO
...
DROP TABLE [RawDataImports];
GO

EXEC sp_rename N'[ImportedDataItems].[IX_ImportedDataItems_ImportedDataRecordId]', N'IX_ImportedDataItems_RecordId', N'INDEX';
GO

ALTER TABLE [ImportedDataRecords] ADD [CompressedData] varbinary(max) NULL;
GO

ALTER TABLE [ImportedDataRecords_History] ADD [CompressedData] varbinary(max) NULL;
GO

ALTER TABLE [ImportedDataRecords] ADD [CompressionRatio] float(5) NOT NULL DEFAULT 0.0E0;
GO

ALTER TABLE [ImportedDataRecords_History] ADD [CompressionRatio] float(5) NOT NULL DEFAULT 0.0E0;
GO

ALTER TABLE [ImportedDataRecords] ADD [SysEndTime] datetime2 NOT NULL DEFAULT '9999-12-31T23:59:59.9999999';
GO

ALTER TABLE [ImportedDataRecords] ADD [SysStartTime] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';
GO

ALTER TABLE [ImportedDataItems] ADD [SysEndTime] datetime2 NOT NULL DEFAULT '9999-12-31T23:59:59.9999999';
GO

ALTER TABLE [ImportedDataItems] ADD [SysStartTime] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';
GO
...
ALTER TABLE [ImportedDataRecords] ADD PERIOD FOR SYSTEM_TIME ([SysStartTime], [SysEndTime])
GO

ALTER TABLE [ImportedDataRecords] ALTER COLUMN [SysStartTime] ADD HIDDEN
GO

ALTER TABLE [ImportedDataRecords] ALTER COLUMN [SysEndTime] ADD HIDDEN
GO

ALTER TABLE [ImportedDataItems] ADD PERIOD FOR SYSTEM_TIME ([SysStartTime], [SysEndTime])
GO

ALTER TABLE [ImportedDataItems] ALTER COLUMN [SysStartTime] ADD HIDDEN
GO

ALTER TABLE [ImportedDataItems] ALTER COLUMN [SysEndTime] ADD HIDDEN
GO

DECLARE @historyTableSchema sysname = SCHEMA_NAME()
EXEC(N'ALTER TABLE [ImportedDataRecords] SET (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [' + @historyTableSchema + '].[ImportedDataRecords_History]))')

GO

DECLARE @historyTableSchema sysname = SCHEMA_NAME()
EXEC(N'ALTER TABLE [ImportedDataItems] SET (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [' + @historyTableSchema + '].[ImportedDataItems_History]))')

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250624074404_AddMissingImportedDataColumns', N'8.0.5');
GO
