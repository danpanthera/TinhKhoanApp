...
# Script kiểm tra và so sánh cấu trúc bảng GL01 với CSV file
# Created: July 12, 2025

echo "🔍 KIỂM TRA CẤU TRÚC BẢNG GL01 vs CSV FILE"
echo "=========================================="
...
echo "$CSV_COLS" > /tmp/csv_cols_gl01.txt
echo "$MODEL_COLS" > /tmp/model_cols_gl01.txt

if diff /tmp/csv_cols_gl01.txt /tmp/model_cols_gl01.txt > /dev/null; then
    echo "✅ PERFECT: Tất cả cột business trong model khớp với CSV!"
else
    echo "❌ KHÁC BIỆT được phát hiện:"
    echo "   CSV có nhưng Model không có:"
    comm -23 /tmp/csv_cols_gl01.txt /tmp/model_cols_gl01.txt | sed 's/^/   - /'
    echo "   Model có nhưng CSV không có:"
    comm -13 /tmp/csv_cols_gl01.txt /tmp/model_cols_gl01.txt | sed 's/^/   - /'
fi

# Dọn dẹp
rm -f /tmp/csv_cols_gl01.txt /tmp/model_cols_gl01.txt

echo ""
echo "📊 KẾT QUẢ CUỐI CÙNG:"
if [ "$CSV_COUNT" -eq "$MODEL_COUNT" ]; then
    echo "✅ PERFECT: Bảng GL01 có đủ $MODEL_COUNT cột business theo file CSV!"
