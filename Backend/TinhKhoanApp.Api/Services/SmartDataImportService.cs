        ...
        public async Task<SmartImportResult> ImportFileSmartAsync(IFormFile file, string? category = null)
        {
            ...
            try
            {
                ...
                if (directResult.Success)
                {
                    ...
                    _logger.LogInformation("✅ Smart import successful: {Records} records to {Table}",
                        directResult.ProcessedRecords, directResult.DataType);
                }
                else
                {
                    _logger.LogWarning("❌ Smart import failed: {Error}", directResult.ErrorMessage);
                }
            ...
            {
                _logger.LogError(ex, "❌ Error in smart import for file: {FileName}", file.FileName);
