...
namespace TinhKhoanApp.Api.Services
{
    ...
    public class DPDAService : IDPDAService
    {
        ...
        /// Sử dụng DirectImportService với CSV structure checking
        /// </summary>
        public async Task<ApiResponse<DPDAImportResultDto>> ImportFromCsvAsync(IFormFile file, string uploadedBy)
        {
            try
            {
                ...
                var importResult = await _directImportService.ImportDPDAFromCsvAsync(file, uploadedBy);
                ...
                var resultDto = new DPDAImportResultDto
                {
                    ...
                    Errors = importResult.Errors?.ToList() ?? new List<string>(),
                    Warnings = importResult.Warnings?.ToList() ?? new List<string>(),
                    RowErrors = importResult.RowErrors?.ToDictionary(x => x.Key, x => x.Value) ?? new Dictionary<int, string>()
                };
        ...
        public async Task<ApiResponse<DPDADetailsDto>> CreateAsync(DPDACreateDto createDto)
        {
            try
            {
                ...
                var createdEntity = await _dpdaRepository.AddAsync(entity);
